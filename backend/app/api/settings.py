...
def get_current_settings(current_user = Depends(require_permission("settings")), db: Session = Depends(get_db)):
...
def update_current_settings(settings_update: SettingsUpdate, current_user = Depends(require_permission("settings")), db: Session = Depends(get_db)):
